...
CAMBIO DE LIBRERIA (importante): este modulo usaba `nfl_data_py`
originalmente. Se migro a `nflreadpy` porque `nfl_data_py` fue
...
(palabras del propio equipo de nflverse). En produccion, `nfl_data_py`
lanzaba un NameError interno ("name 'Error' is not defined") al
procesar datos de la temporada 2026 - un bug real de la libreria que
nadie va a corregir porque esta archivada. `nflreadpy` es el reemplazo
...
Se convierte con `.to_pandas()` justo despues de descargar, y de ahi
en adelante todo el modulo sigue trabajando en pandas sin cambios -
la logica de agregacion por equipo ya estaba probada y se conservo.

CORRECCION respecto al borrador original: el conteo de partidos jugados
debia ser POR EQUIPO, no un promedio de liga. Un promedio de liga permite
que un equipo con muestra real chica (ej. 2 partidos) pase el filtro de
seguridad MIN_SAMPLE_GAMES=4 usando el numero de otro equipo - exactamente
...
~48h despues de cada partido (no es un feed en vivo). No es un riesgo de
leakage hacia partidos futuros, pero si implica que el dato del partido
de HOY puede no estar disponible hasta 1-2 dias despues.
"""
...
# Columnas minimas necesarias - reduce la descarga frente al PBP completo
# (~370 columnas normalmente). Estas son las unicas que usa este modulo.
...
# configurable, pero este dict es independiente y explicito). TTL corto
# porque nflverse solo actualiza cada ~48h, pero evita recalcular en
# cada peticion HTTP mientras el proceso vive.
_CACHE: dict[int, tuple[float, dict, dict]] = {}
_CACHE_TTL_SECONDS = 3600  # 1 hora

LAST_ERROR: Optional[str] = None  # el texto real de la ultima excepcion,
...
def get_team_epa_stats(year: Optional[int] = None,
    ...
    y defensivo POR EQUIPO, junto con el numero real de partidos jugados
    POR EQUIPO (no un promedio de liga).

    use_cache=True (default): reutiliza el resultado si se pidio el mismo
    ano hace menos de _CACHE_TTL_SECONDS. Evita descargar/recalcular en
    cada peticion HTTP del endpoint.
    ...
    inventados. El llamador debe tratar un resultado vacio como NO_DATA,
    ...
    try:
        ...
        # nfl_data_py) - se selecciona con pandas despues de convertir.
        # .to_pandas(): nflreadpy usa Polars internamente; el resto de
        # este modulo ya esta probado sobre pandas, asi que se convierte
        # aqui y se reutiliza esa logica sin cambios.
    ...
    if pbp.empty:
        LAST_ERROR = f"nflverse devolvio 0 filas para la temporada {year} (aun no hay datos publicados, o el ano es incorrecto)"
        print(f"[nfl_data_loader] Sin datos para la temporada {year} todavia.")
        return {}, {}
    ...
    # o playoffs distorsionaria esa calibracion.
    ...
    if valid_plays.empty:
        ...
        print(f"[nfl_data_loader] Sin jugadas validas para {year}/{season_type}.")
        return {}, {}
...
if __name__ == "__main__":
    ...
    if not stats:
        print("Sin datos (revisa conexion, ano, o si la temporada ya empezo).")
    else:
