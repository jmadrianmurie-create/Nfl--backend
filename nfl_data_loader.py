...
se verificó y da 404; el repo fue descontinuado):
  ...
  3. cache=False (descarga directa del repo de GitHub de nflverse en
     cada llamada). CORRECCIÓN: cache=True en nfl_data_py NO significa
     "guardar en disco para la próxima vez" — significa "leer de un
     caché local que debe existir de antemano vía nfl.cache_pbp()".
     Sin haber corrido cache_pbp() primero, cache=True truena con
     ValueError: "<year> cache file does not exist". El cache real de
     este módulo es el diccionario en memoria de abajo (_CACHE), que sí
     evita redescargar en cada request HTTP.
...
# Cache en memoria del PROCESO (no del disco — nfl_data_py usa cache=False,
# ver corrección arriba). TTL corto porque nflverse solo actualiza cada ~48h,
