...
# En produccion, restringir ALLOWED_ORIGIN al dominio real de la app.
...
# En local puede quedar vacia; al desplegar es obligatoria.
WRITE_KEY = os.environ.get("TIPSTER_API_KEY", "")

# Limite de tamano: un payload de prediccion son unos pocos KB.
...
init_db()   # Debe correr SIEMPRE al importar el modulo, no solo en
            # ejecucion directa: gunicorn (usado por Render) importa
            # app.py como modulo y nunca entra al bloque __main__, asi
            # que dejarlo solo ahi significa que la base nunca se crea
            # en produccion. executescript() es seguro de repetir:
