...
# ================= MLB StatsAPI - respaldo para la app =================
# La app pide los datos de MLB directo a statsapi.mlb.com. Si el teléfono no
# puede (red, bloqueo del navegador), reintenta aquí y este servidor los trae.
# Solo lectura, solo dos recursos, sin clave: no expone nada tuyo.
import urllib.request as _urlreq
from urllib.parse import urlencode as _urlencode
from flask import Response as _Response

_MLB_ALLOWED = {"schedule", "standings"}
_MLB_PARAMS = {"sportId", "date", "hydrate", "leagueId", "season", "standingsTypes"}


@app.get("/api/mlb/<resource>")
def mlb_proxy(resource):
    if resource not in _MLB_ALLOWED:
        return jsonify({"error": "recurso no permitido"}), 404
    params = {k: v for k, v in request.args.items() if k in _MLB_PARAMS and len(v) <= 40}
    url = f"https://statsapi.mlb.com/api/v1/{resource}?{_urlencode(params)}"
    try:
        req = _urlreq.Request(url, headers={"User-Agent": "tipster-backend"})
        with _urlreq.urlopen(req, timeout=20) as r:
            body = r.read()
        return _Response(body, status=200, mimetype="application/json")
    except Exception as e:
        return jsonify({"error": "MLB StatsAPI no respondió", "detail": f"{type(e).__name__}: {e}"}), 502
